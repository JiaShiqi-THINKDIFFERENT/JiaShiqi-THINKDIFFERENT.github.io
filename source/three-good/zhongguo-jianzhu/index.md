---
title: 三好分析 · 中国建筑
date: 2026-09-24 17:37
---

# 三好投资法则分析 · 中国建筑（601668）

> 依据邱国鹭《投资中最简单的事》"三好原则"（好行业 × 好公司 × 好价格）+ 量化硬模型，
> 每周四收盘后自动更新。定性层为人工一次性评估（每季度复核），量化层每周由脚本重算。

## 分析概要（2026-09-24）

| 项目 | 值 |
| ---- | ---- |
| 现价（前复权） | 4.26 元 |
| PE-TTM 十年分位 | 66.8% |
| PB 十年分位 | 0.5% |
| 股息率分位 | 99.4% |
| **综合评分** | **51.4 / 100 — 🟠 观望** |
| 行业分（30%） | 61 |
| 公司分（35%） | 46.6 |
| 价格分（25%） | 75.2 |


## 好行业（61 / 100）

| 维度 | 得分 | 满分 |
| ---- | ---- | ---- |
| 行业格局 | 18 | 25 |
| 定价权 | 12 | 25 |
| 需求稳定性 | 14 | 25 |
| 进入壁垒 | 17 | 25 |

> 全球最大的房建与基建承包商，订单规模与央企资质构成实质壁垒，资源获取与融资能力远超同业
> 业务结构正从纯施工向基建投资与运营倾斜。但施工环节毛利率天然偏低，垫资模式导致应收账款与合同资产规模庞大，减值侵蚀利润
> 地产链敞口进一步拖累回款。议价能力弱，几乎是价格接受者。管理层稳健，ROE 长期偏低是结构性难题。

## 好公司（46.6 / 100）

**护城河（定性，16 / 40）+ 管理层/资本配置（定性，14 / 20）+ 时序校准（+0.0）**

财务质量评分卡（量化硬模型，16.6 / 30）：

| 模型 | 结果 | 得分 |
| ---- | ---- | ---- |
| Piotroski F-Score | F-Score 4/5(可得项归一) | 9.6 / 12 |
| 价值创造（5-10年 ROE 均值替代 ROIC） | ROE均值 7.9%，偏低 | 2.0 / 8 |
| 盈余质量（经营现金流/净利） | 现金含量 0.53，一般 | 5.0 / 10 |

时序趋势：时序趋势中性

## 好价格（75.2 / 100）

| 项 | 得分 | 说明 |
| ---- | ---- | ---- |
| PE-TTM 十年分位（0-40） | 15.2 | 分位 66.8% |
| PB 十年分位（0-30） | 30.0 | 分位 0.5% |
| 股息率分位（0-15） | 15.0 | 股息率分位 99% |
| 绝对 PE 锚（0-15） | 15.0 | PE-TTM 5.6 倍 |

## 一票否决检查

- ✅ 未触发（M-Score / Z-Score 在 A 股摘要级数据下多为不适用，按 skill 护栏仅 applicable=True 时启用）

## 近三个月评分趋势

| 日期 | 综合 | 行业 | 公司 | 价格 | 结论 |
| ---- | ---- | ---- | ---- | ---- | ---- |
| 2026-09-24 | 51.4 | 61 | 46.6 | 75.2 | 🟠 观望 |

## 口径与局限

- 量化层由 `tools/value-analysis/score_stock.py` 调用 stock-value-analyzer skill 的
  `fetch_stock_data.py`（akshare A 股引擎）+ `value_models.py` 自动计算，可复现。
- A 股摘要级财务数据缺三大报表明细：M-Score / Z-Score / 反向 DCF 多数不适用（skill 护栏下不作否决）；
  ROIC 用 5-10 年 ROE 均值替代；F-Score 按可得项归一。
- 行业分/护城河/管理层为定性评分，配置存于 `tools/value-analysis/qualitative.json`，每季度人工复核。
- 定价权专项调整：-2 分。
- 本报告由自动化流水线生成，仅供研究参考，不构成投资建议。
